...
class Toddler:
    ...
    def correct_heading(self, dist_last_travelled, desired_wall_dist):
        ...
        if dist_last_travelled > minimum_dist_to_travel:
            self.curr_small_ang_dev = np.arcsin(
                (curr_wall_dist - self.old_wall_dist) / dist_last_travelled)
            correction_heading = np.arctan(
                (curr_wall_dist - desired_wall_dist) / self.wallwalker.get_step_size())
            print('last_dist_travelled {}'.format(dist_last_travelled))
            print('Distance from the wall {} / {} / {}'.format(self.old_wall_dist,
                                                               curr_wall_dist, desired_wall_dist))
            print('Correcting heading {} / {} / {}'.format(self.curr_small_ang_dev,
                                                           correction_heading, self.curr_small_ang_dev + correction_heading))
            self.motion.turn(
                (self.curr_small_ang_dev + correction_heading) * 0.9)
        else:
    ...
    def correct_heading2(self, dist_last_travelled, desired_wall_dist):
        ...
        print('dist: {} / {}, error {}%'.format(curr_wall_meas,
                                                desired_wall_dist, np.abs(wall_error) / desired_wall_dist))
        if np.abs(wall_error) / desired_wall_dist >= allowed_wall_error:
            print('correcting heading')
            angl_to_straight = np.arcsin(
                wall_error / self.dist_since_being_accurate)
            dist_along_wall_since_last_corr = self.dist_since_being_accurate * \
                np.cos(angl_to_straight)
            dist_along_wall_to_go = self.wallwalker.get_segment_len() - \
                dist_along_wall_since_last_corr
            angl_to_correct = np.arctan(wall_error / dist_along_wall_to_go)
            print('angl_to_straight {}\tdist_along_wall_since_last_corr {}\tdist_along_wall_to_go {}\tangl_to_correct {}'.format(
                angl_to_straight, dist_along_wall_since_last_corr, dist_along_wall_to_go, angl_to_correct))
